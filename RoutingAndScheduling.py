...
def surreal():
    g = nx.Graph()
    for i, elrow in mDB.silk_edge_list.iterrows():
        g.add_edge(elrow[0], elrow[1], weight=elrow[3], attr_dict=elrow[3:].to_dict())
    for i, nlrow in mDB.silk_node_list.iterrows():
...
def shortest_path_length(spider_location, prey_location):
    return round(nx.dijkstra_path_length(web, source=spider_location, target=prey_location), 2)
...
def hunting(c_time):
    ...
    spiders = gb.waiting_spider_list.loc[gb.waiting_spider_list['Scheduling Time'].dt.date
                                         == c_time.date()].sort_values(by='Scheduling Time')
...
def attack(spider_id, preys_for_spider, last_station_center_of_web, c_time):
    ...
    journey_starting_time = gb.waiting_spider_list['Journey Started'].loc[
        (gb.waiting_spider_list['Spider-Id'] == spider_id) & (gb.waiting_spider_list['Scheduling Time'].dt.date
    ...
    gb.waiting_spider_list.loc[(gb.waiting_spider_list['Spider-Id'] == spider_id) &
                               ...
                               ['X', 'Y', 'Available', 'Waiting From Time', 'Spider in Allocation',
                                'Journey Started', 'Spider-Location-Id']] = \
        s_x, s_y, False, updated_waiting_from_time, False, journey_starting_time, spider_loc_id
...
def routing(spider_id, preys_for_spider_ids, c_time):  # prey list as parameter
    ...
    for prey_node in preys_for_spider_ids:
        ...
        for node_id in list_of_silk_node_visited:
            mDB.spider_nodes_visited = mDB.spider_nodes_visited.append(
                pd.DataFrame([[spider_id, c_time.date(), node_id, trip_no]], columns=['Spider-Id',
                                                                                      'Date', 'Node-Id', 'Trip No']))
        ...
        time_consumed_in_this_trip = time_consumed_in_this_trip + \
                                     dt.timedelta(seconds=round(distance_travelled/gb.avg_speed, 2))
